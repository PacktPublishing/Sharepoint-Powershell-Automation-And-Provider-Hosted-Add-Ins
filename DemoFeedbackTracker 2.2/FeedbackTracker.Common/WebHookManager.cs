    ...
    public class WebHookManager
    {
        ...
        public async Task<SubscriptionModel> AddListWebHookAsync(string siteUrl, string listId, string webHookEndPoint, string accessToken, int validityInMonths = 3)
        {
            ...
            using (var httpClient = new HttpClient())
            {
                ...
                //throw new Exception($"siteUrl {siteUrl} listid {listId} at {accessToken} requestUrl {requestUrl} webHookEndPoint {webHookEndPoint}");

                HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Post, requestUrl);
                request.Headers.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                request.Headers.Authorization = new AuthenticationHeaderValue("Bearer", accessToken);

                var sm = new SubscriptionModel()
                {
                ...
                request.Content = new StringContent(JsonConvert.SerializeObject(sm),
                    Encoding.UTF8, "application/json");

               // throw new Exception($"{sm.Resource} {sm.NotificationUrl} {sm.ExpirationDateTime} ");
